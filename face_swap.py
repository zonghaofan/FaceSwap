...
def warp_image_3d(src_img, src_points, dst_points, dst_shape, dtype=np.uint8):
    ...
    # print('delaunay.simplices.shape',delaunay.simplices.shape)
    # plt.triplot(dst_points[:, 0], dst_points[:, 1], delaunay.simplices.copy())
    # plt.show()
    tri_affines = np.asarray(list(triangular_affine_matrices(
        delaunay.simplices, src_points, dst_points)))
    print('tri_affines.shape',tri_affines.shape)
...
def mask_from_points(size, points,erode_flag=1):
    ...
    # [luzi]: change y axises of eyebows to zero

    mask = np.zeros(size, np.uint8)
    cv2.fillConvexPoly(mask, cv2.convexHull(points), 255)

    """debug for convexHull"""
    # mask = np.expand_dims(mask, axis=-1)
    # mask = np.concatenate((mask, mask, mask), axis=-1).astype(np.float32)
    # print('mask.shape=', mask.shape)
    # for i in points:
    #     cv2.circle(mask, tuple(i), 5, (0, 255, 0), -1)
    # for i in np.squeeze(cv2.convexHull(points)):
    #     cv2.circle(mask, tuple(i), 6, (0, 0, 255), -1)
    # cv2.imshow('mask_convex',mask)
    # cv2.waitKey(0)
...
def correct_colours(im1, im2, landmarks1):
    ...
    if blur_amount % 2 == 0:
        blur_amount += 1  #origin: blur_amount += 1
